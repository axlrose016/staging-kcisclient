...
import { ILibAllotmentClass, ILibAppropriationSource, ILibAppropriationType, ILibBudgetYear, ILibEmploymentStatus } from "@/components/interfaces/library-interface"
import { LibrariesService } from "@/components/services/LibrariesService"

const formSchema = z.object({
  id: z.number().optional(),
  allotment_class_description: z.string().min(1, "Allotment Class is Required"),
});

type FormValues = z.infer<typeof formSchema>
const libService = new LibrariesService();

export default function FormAllotmentClass() {
  ...
  function onSubmit(data: FormValues) {
    ...
    libService.saveOfflineLibAllotmentClass(data).then((response:any) => {
      if (response) {
        router.push(`/${baseUrl}/`);
        toast({
            variant: "green",
  ...
  return (
    <div className="container mx-auto py-10">
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          <CardTitle>Allotment Class</CardTitle>
          <CardDescription>Enter or update the Allotment Class.</CardDescription>
        </CardHeader>
        <Form {...form}>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
                <FormField
                  ...
                  name="allotment_class_description"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Allotment Class</FormLabel>
                      <FormControl>
                        <Input placeholder="Enter Allotment Class" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
