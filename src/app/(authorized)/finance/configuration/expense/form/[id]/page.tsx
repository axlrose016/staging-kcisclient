...
import { ILibBudgetYear, ILibEmploymentStatus, ILibExpense } from "@/components/interfaces/library-interface"
import { LibrariesService } from "@/app/(authorized)/library/LibrariesService"

const formSchema = z.object({
  id: z.number().optional(),
  expense_code: z.string().min(1, "Expense Code is Required"),
  expense_description: z.string().min(1, "Expense Description is Required")
});
...
export default function FormExpense() {
  ...
  function onSubmit(data: FormValues) {
    ...
    libService.saveOfflineLibExpense(data).then((response:any) => {
      if (response) {
        router.push(`/${baseUrl}/`);
        toast({
            variant: "green",
    ...
    <div className="container mx-auto py-10">
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>Enter or update the Expense.</CardDescription>
        </CardHeader>
        <Form {...form}>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Enter Expense Code" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
