...
function ListAppropriationSource() {
    ...
    async function loadAppropriationSource(){
        try{
            const data = await libService.getOfflineAppropriationSource() as any;
            setData(data);
        }catch(error){
            console.error(error);
        }finally{
    ...
    const handleDelete = async (row: any) => {
        const success = await settingsService.deleteData(libDb, "lib_appropriation_source", row);
        if (success) {
        toast({
            variant: "green",
            title: "Success.",
...
export default ListAppropriationSource